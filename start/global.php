...
*/
 ...
 /**
  * This function handles methods that are not allowed
  */
...
{   
  // Always send error to Rollbar
  Log::error($exception);

  // Clearly this is some kind of custom logging functionality for the API
  if (starts_with(Request::path(), 'api')){
    ...
    $logString = date('Y-m-d H:i:s').PHP_EOL.''.$request->getMethod().' || '. $request->getURI().' || '.$request->getContent().PHP_EOL.
                 $code.' || '.$exception->getTraceAsString().PHP_EOL.PHP_EOL;
...
App::missing(function()
 {
    if (starts_with(Request::path(), 'api')){
        return Response::make(array('error'=>Config::get('api::responseConstants.invalidRoute')),404);
    }
 });
